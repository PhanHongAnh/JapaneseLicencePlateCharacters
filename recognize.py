...
class Recognize:
    def __init__(self, model):
        ...
        self.kanji_clf = pickle.load(open("Kanji_SVC.pickle", "rb"))
    ...
    def hog_in_rect(self, rect):
        ...
        im,thresh = cv2.threshold(roi,127,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    ...
    def recognize_in_rect(self, rect, choice):
        ...
        if (choice == 1):
            ...
            char = label.search_hira(int(nbr[0]))
        elif (choice == 3):
            nbr = self.kanji_clf.predict(np.array([roi_hog_fd], np.float32))
            char = label.search_kanji(int(nbr[0]))
        return char
